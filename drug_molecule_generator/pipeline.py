...
def encoder_decoder_with_attn_mech():
    ...
    dec_embedding = Embedding(input_dim=len(config.BASE_VOCABULARY)+2,output_dim=(len(config.BASE_VOCABULARY)+2)//2,input_length=config.MAX_OUTPUT_SEQUENCE_LEN,
    ...
    dec_output = Dense(units=len(config.BASE_VOCABULARY)+2,activation="softmax",name="decoder_output")(dec_dense_input)
